#!/usr/bin/env bash
# autopilot-check.sh — Hook script for PMLORD autopilot transition evaluation
...
# Example: ./autopilot-check.sh RFQ-2026-0042 pmlord-rfq-intake
#
# Exit codes:
...
# Strip the "pmlord-" prefix for transition rule matching
SHORT_SKILL=$(echo "$COMPLETED_SKILL" | sed 's/^pmlord-//')
...
if [ "$AUTO_FLAG" = "PAUSE" ]; then
    echo '{"entity_id":"'"$ENTITY_ID"'","completed":"'"$COMPLETED_SKILL"'","next":"pmlord-'"$NEXT_SKILL"'","action":"PAUSE","reason":"transition_requires_approval","mode":"'"$AUTOPILOT_MODE"'","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}'
    echo '{"ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","entity_id":"'"$ENTITY_ID"'","from":"'"$COMPLETED_SKILL"'","to":"pmlord-'"$NEXT_SKILL"'","trigger":"autopilot_pause","reason":"transition_requires_approval","mode":"'"$AUTOPILOT_MODE"'"}' >> "$WORKFLOW_FILE" 2>/dev/null || true
    exit 1
fi
...
if [ "$AUTO_FLAG" = "AUTO" ]; then
    ...
    echo '{"entity_id":"'"$ENTITY_ID"'","completed":"'"$COMPLETED_SKILL"'","next":"pmlord-'"$NEXT_SKILL"'","action":"AUTO_ADVANCE","mode":"'"$AUTOPILOT_MODE"'","cancel_window_minutes":'"$CANCEL_WINDOW"',"ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}'
    echo '{"ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","entity_id":"'"$ENTITY_ID"'","from":"'"$COMPLETED_SKILL"'","to":"pmlord-'"$NEXT_SKILL"'","trigger":"autopilot","mode":"'"$AUTOPILOT_MODE"'","cancel_window":'"$CANCEL_WINDOW"'}' >> "$WORKFLOW_FILE" 2>/dev/null || true
    exit 0
fi
