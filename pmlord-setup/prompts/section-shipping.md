...
Configure shipping carriers, customs brokers, origin/destination ports, incoterms, insurance, and freight forwarders. This data drives `pmlord-logistics`, `pmlord-export-compliance`, and `pmlord-import-compliance`.
...
Write to `~/.pmlord/config/shipping-config.yaml` using `templates/shipping-config.yaml.tmpl`.
