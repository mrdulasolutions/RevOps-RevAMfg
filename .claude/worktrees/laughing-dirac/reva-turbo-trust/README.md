...
Progressive autonomy system for the REVA-TURBO skills engine.
...
```
/reva-turbo-trust set 2
```

### Run Trust Assessment

```
/reva-turbo-trust assess
```
...
Per-skill override:
```
/reva-turbo-trust override --skill reva-turbo-export-compliance --level 1
```

Per-entity override:
```
/reva-turbo-trust override --entity customer:acme-corp --level 1
```
...
All trust state is stored at `~/.reva-turbo/state/`:
...
Per-user overrides: `~/.reva-turbo/users/<pm-slug>/trust-level.json`

## Integration

Every REVA-TURBO skill should call `bin/trust-check.sh` at startup to determine the effective trust level, then apply the behavioral overlay from the corresponding prompt file (`prompts/trust-learn.md`, `prompts/trust-assist.md`, or `prompts/trust-operate.md`).
