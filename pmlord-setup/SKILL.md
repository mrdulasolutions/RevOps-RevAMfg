...
version: 1.0.0
description: |
  Interactive onboarding wizard for PMLORD engine configuration.
...
```bash
mkdir -p ~/.pmlord/analytics ~/.pmlord/config ~/.pmlord/sessions ~/.pmlord/state
echo '{"skill":"pmlord-setup","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true

# Detect existing config for resume
_SETUP_COMPLETE=$(~/.claude/skills/pmlord/bin/pmlord-config get setup_completed 2>/dev/null || echo "false")
_HAS_COMPANY=$([ -f ~/.pmlord/config/company-profile.yaml ] && echo "yes" || echo "no")
_HAS_WORKFLOW=$([ -f ~/.pmlord/config/workflow-config.yaml ] && echo "yes" || echo "no")
_HAS_CONNECTORS=$([ -f ~/.pmlord/config/connector-config.yaml ] && echo "yes" || echo "no")
_HAS_PARTNERS=$([ -f ~/.pmlord/config/partners.yaml ] && echo "yes" || echo "no")
_HAS_SHIPPING=$([ -f ~/.pmlord/config/shipping-config.yaml ] && echo "yes" || echo "no")
_HAS_DOCUMENTS=$([ -f ~/.pmlord/config/document-config.yaml ] && echo "yes" || echo "no")
...
PMLORD Setup is an interactive 7-section onboarding wizard that configures the entire PMLORD engine for a specific company. It creates 6 YAML configuration files at `~/.pmlord/config/` that drive every downstream skill — from RFQ intake through customer delivery.
...
1. If `SETUP_COMPLETE` is `true`, inform the PM:
   > PMLORD setup is already complete. What would you like to do?
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  PMLORD Setup — Section X of 7: [Name]
  ██████░░░░░░░░ X/7 complete
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**Writes:** `~/.pmlord/config/company-profile.yaml`
...
If A, render `templates/company-profile.yaml.tmpl` with collected values and write to `~/.pmlord/config/company-profile.yaml`.
...
**Writes:** `~/.pmlord/config/connector-config.yaml`
...
2. **Email**:
   ...
   - Sending address (the "from" address for PMLORD emails)
...
**Security note:** Never write API keys, tokens, or passwords directly to config files. Instead, write environment variable references like `${PMLORD_HUBSPOT_API_KEY}`. Instruct the PM to set these in their shell profile.
...
**Writes:** `~/.pmlord/config/partners.yaml`
...
**Writes:** `~/.pmlord/config/shipping-config.yaml`
...
**Writes:** `~/.pmlord/config/document-config.yaml`
...
3. **Report footer text** — e.g., "Generated by PMLORD | {date}"
4. **Default output format**: A) .docx, B) .pdf, C) Both
5. **Template overrides directory** — path for custom template files (default: `~/.pmlord/templates/`)
6. **Output storage location** — where to save generated reports:
   - Local path (default: `~/Documents/PMLORD-Reports/`)
...
8. **Document naming convention** — confirm or customize:
   - Default: `PMLORD-{Type}-{YYYY-MM-DD}-{ShortName}.docx`
...
1. **Explain CoWork organization** for PMLORD:
   > Claude CoWork lets you organize AI-assisted projects into separate workspaces. For PMLORD, we recommend:
...
2. **Auto-create starter projects** — offer to create recommended project names:
   - "PMLORD — Internal Ops"
   - "PMLORD — Active RFQs"
   - "PMLORD — [Customer Name]" (for each team member's top customer)
...
1. **Set setup flag**:
```bash
~/.claude/skills/pmlord/bin/pmlord-config set setup_completed true 2>/dev/null || true
~/.claude/skills/pmlord/bin/pmlord-config set setup_date "$(date -u +%Y-%m-%dT%H:%M:%SZ)" 2>/dev/null || true
```
...
for f in company-profile workflow-config connector-config partners shipping-config document-config; do
  if [ -f ~/.pmlord/config/${f}.yaml ]; then
    bash "${CLAUDE_SKILL_DIR}/bin/validate-config.sh" ~/.pmlord/config/${f}.yaml
  fi
done
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  PMLORD Setup Complete!
  ██████████████ 7/7 sections configured
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Config Files Created:
  ✓ ~/.pmlord/config/company-profile.yaml
  ✓ ~/.pmlord/config/workflow-config.yaml
  ✓ ~/.pmlord/config/connector-config.yaml
  ✓ ~/.pmlord/config/partners.yaml
  ✓ ~/.pmlord/config/shipping-config.yaml
  ✓ ~/.pmlord/config/document-config.yaml

  Skipped Sections: [list any skipped]

  Next Steps:
  1. Run /pmlord-dashboard to see your engine status
  2. Run /pmlord-rfq-intake to process your first RFQ
  3. Run /pmlord-setup section:N to reconfigure any section
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

4. **Log completion**:
```bash
echo '{"event":"setup_complete","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","sections_completed":"'$_COMPLETED'","sections_skipped":"'$_SKIPPED'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
1. Record the skip: `~/.claude/skills/pmlord/bin/pmlord-config set "section_N_skipped" "true"`
...
3. **Backup before overwrite** — Copy existing config to `~/.pmlord/config/backups/` with timestamp before replacing
