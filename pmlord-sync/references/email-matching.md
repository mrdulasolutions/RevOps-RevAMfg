...
How to match incoming emails to PMLORD entities: orders, customers, partners, and workflow actions.
...
| Sender name fuzzy match | 3 | "John Smith" fuzzy matches a contact name in PMLORD |
| Reply-to thread | 4 | Email is a reply to a thread PMLORD already tracks |
...
|---------|-------|-------------|
| PMLORD Order ID | `ORD-\d{4}-\d{3,4}` | Order |
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","from":"{{FROM}}","subject":"{{SUBJECT}}","classification":"unmatched","status":"pending_review"}' >> ~/.pmlord/state/email-queue.jsonl
```

PM reviews the queue periodically and either:
- Manually links to a PMLORD entity
