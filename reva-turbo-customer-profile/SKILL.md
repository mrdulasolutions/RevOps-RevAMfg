---
name: reva-turbo-customer-profile
preamble-tier: 2
version: 1.0.0
description: |
...
echo '{"skill":"reva-turbo-customer-profile","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
ls ~/.reva-turbo/customers/ 2>/dev/null
```
...
If A and no onboarding record exists, recommend running `reva-turbo-customer-gate` first.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","customer":"{{CUSTOMER_COMPANY}}","customer_id":"{{CUSTOMER_ID}}","stage":"reva-turbo-customer-profile","status":"{{PROFILE_ACTION}}","pm":"{{ASSIGNED_PM}}"}' >> ~/.reva-turbo/state/workflow-state.jsonl 2>/dev/null || true
```
...
```
REVA-TURBO-CustProfile-{YYYY-MM-DD}-{CustomerShortName}.md
```
