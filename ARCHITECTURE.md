...
Two settings: `off` (default), `on`.
Events logged locally to `~/.reva-turbo/analytics/skill-usage.jsonl` in JSONL format.
Managed by `bin/reva-turbo-telemetry-log`. Nothing is sent externally.
