...
**Universal Email Connector for PMLORD**

Connects to any email provider available in the Claude instance. Auto-detects connected inboxes, reads and classifies incoming emails, routes them to the appropriate PMLORD skill, and handles outbound sending/replying.
...
```
/pmlord-email-connector              # Scan inbox (auto-detect provider)
/pmlord-email-connector mode:send    # Send email via configured provider
/pmlord-email-connector mode:reply   # Reply to a thread
/pmlord-email-connector mode:search  # Search across all connected inboxes
/pmlord-email-connector mode:setup   # Set up a new email provider
```

Or ask PMLORD naturally:
...
5. **Route** confirmed emails to the appropriate PMLORD skill
6. **Log** all routing decisions to `~/.pmlord/state/email-routing-log.jsonl`
...
| `send` | Send an email (used by pmlord-customer-comms and other skills) |
...
PMLORD can connect to multiple email providers simultaneously. When scanning:
...
Configure in `~/.pmlord/connector-config.yaml` (via `/pmlord-setup section:3`).
...
| File | Purpose |
|------|---------|
| `~/.pmlord/state/email-routing-log.jsonl` | Routing history (sender, subject, category, skill, timestamp) |
| `~/.pmlord/connector-config.yaml` | Provider configuration (set via pmlord-setup) |
