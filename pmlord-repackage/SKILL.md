...
echo '{"skill":"pmlord-repackage","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","po":"{{PO_NUMBER}}","event":"repackage_wo_created","packaging":"{{PACKAGING_TYPE}}","labeling":"{{LABELING_TYPE}}","target_date":"{{TARGET_DATE}}","pm":"{{ASSIGNED_PM}}"}' >> ~/.pmlord/orders/{{PO_NUMBER}}/order-state.jsonl
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","po":"{{PO_NUMBER}}","event":"repackage_complete","completed_by":"{{COMPLETED_BY}}","packages":"{{NUM_PACKAGES}}","ready_for_ship":true}' >> ~/.pmlord/orders/{{PO_NUMBER}}/order-state.jsonl
```

4. Advance order to Stage 09 (Repackaged) in pmlord-order-track
5. Suggest next step: set up domestic shipment via pmlord-logistics

> Repackaging complete for {{PO_NUMBER}} ({{CUSTOMER_COMPANY}}). {{NUM_PACKAGES}} packages ready for shipment.
> Next step: Set up domestic shipping via pmlord-logistics. Proceed?

## Report Naming

Save repackaging work orders as:
```
PMLORD-Repackage-{YYYY-MM-DD}-{PO_NUMBER}.md
```
