...
Business rules engine for the REVA-TURBO system.
...
5. **Evaluate rules** automatically when other REVA-TURBO skills run
...
Rules are stored in `~/.reva-turbo/rules.yaml`. When any REVA-TURBO skill runs (quoting, order tracking, etc.), it calls the rules engine to check for applicable rules. The engine evaluates all active rules against the current context and returns matching actions.
...
- Rules stored in `~/.reva-turbo/rules.yaml`
- Rule changes logged to `~/.reva-turbo/state/rules-log.jsonl`
- Rule evaluations logged to `~/.reva-turbo/state/rules-eval-log.jsonl`
...
Other REVA-TURBO skills integrate with the rules engine by calling:

```bash
echo '{"context": "data"}' | bash ~/.reva-turbo/bin/rules-engine.sh --scope "skill-name"
```
