...
| 4 hours | If still not acknowledged: generate an escalation record via pmlord-escalate |

**Backup PM routing:**
- If a backup PM is configured in `pulse-config.jsonl`, route the alert to them at the 2-hour mark.
- If no backup PM configured, include a note in the escalation: "No backup PM configured. Consider setting one up via /pmlord-pulse configure."
