...
### How It Works
- PM enters data manually when prompted by PMLORD skills
- Data is stored in PMLORD state files as JSON
...
2. Saves the export to `~/.pmlord/imports/`
3. PMLORD parses the CSV and loads the data
...
3. Convert to PMLORD JSON data structures
4. Present summary to PM for confirmation
5. Save to `~/.pmlord/erp-data/`
...
2. Files are placed in `~/.pmlord/imports/` or `~/.pmlord/erp-data/`
3. PMLORD reads and uses the data directly
...
All retrieved ERP data is cached locally at `~/.pmlord/erp-data/` with timestamps. This ensures PMLORD can function even when the ERP system is unavailable.
