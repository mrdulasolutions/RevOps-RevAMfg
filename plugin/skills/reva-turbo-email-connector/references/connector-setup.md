...
Step-by-step instructions for connecting each email provider to REVA-TURBO.
...
3. **Verify:** Run `/reva-turbo:reva-turbo-email-connector mode:setup` — the connector will test the connection.
...
- **Direct Send:** Unlike Gmail, Microsoft connector sends immediately (with PM confirmation in REVA-TURBO)

---

## Multi-Provider Setup

REVA-TURBO supports connecting multiple email providers simultaneously. Common setups:

### Setup 1: Hostinger Primary + Gmail Personal
```yaml
# In ~/.reva-turbo/connector-config.yaml
email:
  provider: "hostinger"
  providers_available:
    - type: hostinger
...
| Emails being processed twice | Check `~/.reva-turbo/state/email-routing-log.jsonl` — dedup uses message_id |
...
3. **Token storage** — MCP servers manage tokens. Don't store tokens in REVA-TURBO config files.
...
6. **No body logging** — REVA-TURBO never stores full email bodies in log files.
