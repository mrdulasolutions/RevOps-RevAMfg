...
echo '{"skill":"pmlord-export-compliance","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
This skill delegates the actual compliance screening to the **ExChek engine** (exchek.us) — a purpose-built export compliance AI engine. PMLORD detects whether ExChek is installed and routes accordingly.
...
>
> PMLORD uses the ExChek engine (exchek.us) for export compliance screening.
...
>    ```
> 4. Re-run `/pmlord-export-compliance` — it will auto-detect the installation
>
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"compliance_skip","gate":"export","reason":"user_opted_out","pm":"{{PM_NAME}}","context":"{{CONTEXT}}"}' >> ~/.pmlord/state/workflow-state.jsonl
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"export_compliance","gate":"CLEARED","eccn":"{{ECCN}}","destination":"{{DESTINATION}}","end_user":"{{END_USER}}","license":"{{LICENSE_STATUS}}","pm":"{{PM_NAME}}","exchek_tier":"{{EXCHEK_TIER}}"}' >> ~/.pmlord/state/workflow-state.jsonl
```
...
Route to `pmlord-escalate` with compliance context. Tag as "Legal / Export Compliance."
...
**Screening ID:** PMLORD-EXP-{{YYYY-MM-DD}}-{{SLUG}}
...
- Logged to: ~/.pmlord/state/workflow-state.jsonl
```

Save as `PMLORD-ExportCompliance-{{YYYY-MM-DD}}-{{SLUG}}.docx` if report_format is docx.
...
```
pmlord-rfq-quote (quote accepted)
  -> pmlord-customer-comms (send confirmation)
    -> *** pmlord-export-compliance *** (HARD GATE — before sending data to China)
      -> pmlord-china-package (only if compliance CLEARED)
        -> pmlord-china-track
          -> ...
            -> *** pmlord-export-compliance *** (if shipping internationally)
              -> pmlord-logistics (only if compliance CLEARED)
```
...
- **pmlord-email-connector:** Compliance clearance notification sent to PM
- **pmlord-crm-connector:** Compliance status field updated in CRM
- **pmlord-pulse:** Alert on HOLD or BLOCK decisions
- **pmlord-audit-trail:** All compliance decisions logged
...
- **Previous skill:** pmlord-rfq-quote (after quote accepted) OR pmlord-repackage (before international shipment)
- **Next skill:** pmlord-china-package (if technical data) OR pmlord-logistics (if physical export)
- **On HOLD/BLOCK:** pmlord-escalate
