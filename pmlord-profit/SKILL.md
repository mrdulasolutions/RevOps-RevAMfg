...
echo '{"skill":"pmlord-profit","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
This skill expects a delivered order with a completed quote (from `pmlord-rfq-quote`) and actual cost data available. If the order has not been delivered, warn the PM:
...
```bash
grep -l "{{ORDER_SEARCH}}" ~/.pmlord/state/workflow-state.jsonl 2>/dev/null || echo "No matching orders found"
```
...
```bash
grep "{{CUSTOMER_COMPANY}}" ~/.pmlord/state/workflow-state.jsonl 2>/dev/null | grep '"stage":"pmlord-profit"' || echo "No previous profit records for this customer"
```
...
```bash
mkdir -p ~/.pmlord/profit-analysis/{{CUSTOMER_COMPANY}}
```

Report naming:

```
PMLORD-Profit-{YYYY-MM-DD}-{CustomerShortName}-{OrderID}.md
```
...
>
> This suggests the estimation guidelines in `pmlord-rfq-quote` need updating.
>
> A) Generate updated margin/cost guidelines (I will draft changes to `references/margin-guidelines.md` and `references/cost-estimation.md` in `pmlord-rfq-quote`)
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","order_id":"{{ORDER_ID}}","customer":"{{CUSTOMER_COMPANY}}","stage":"pmlord-profit","status":"complete","revenue":"{{ACTUAL_REVENUE}}","total_cost":"{{ACT_TOTAL_COST}}","gross_profit":"{{GROSS_PROFIT}}","gross_margin_pct":"{{GROSS_MARGIN_PCT}}","quoted_margin_pct":"{{QUOTED_MARGIN}}","margin_delta":"{{MARGIN_DELTA}}","top_variance":"{{TOP_VARIANCE_CATEGORY}}","top_variance_pct":"{{TOP_VARIANCE_PCT}}","pm":"{{ASSIGNED_PM}}"}' >> ~/.pmlord/state/workflow-state.jsonl 2>/dev/null || true
```

> Profitability analysis complete for {{ORDER_ID}} ({{CUSTOMER_COMPANY}}).
> Gross margin: {{GROSS_MARGIN_PCT}}% (quoted: {{QUOTED_MARGIN}}%).
> Report saved to `~/.pmlord/profit-analysis/{{CUSTOMER_COMPANY}}/`.
