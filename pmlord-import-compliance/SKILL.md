...
echo '{"skill":"pmlord-import-compliance","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
_TI_API_KEY=$(~/.claude/skills/pmlord/bin/pmlord-config get tradeinsights_api_key 2>/dev/null || echo "")
_TI_AVAILABLE="no"
...
>
> PMLORD uses TradeInsights.ai (tradeinsights.ai) for HTS classification
...
```bash
~/.claude/skills/pmlord/bin/pmlord-config set tradeinsights_api_key "{{API_KEY}}"
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"compliance_skip","gate":"import","reason":"user_opted_out","pm":"{{PM_NAME}}"}' >> ~/.pmlord/state/workflow-state.jsonl
```
...
Flag any missing documents and route to `pmlord-logistics` for follow-up.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"import_compliance","gate":"CLEARED","order_id":"{{ORDER_ID}}","hts_codes":["{{HTS_CODES}}"],"total_duties":"{{TOTAL_DUTIES}}","effective_rate":"{{EFFECTIVE_RATE}}","method":"{{CLASSIFICATION_METHOD}}","pm":"{{PM_NAME}}"}' >> ~/.pmlord/state/workflow-state.jsonl
```

Feed duty costs to `pmlord-profit` for actual cost tracking:

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"actual_cost","order_id":"{{ORDER_ID}}","category":"duties_tariffs","amount":{{TOTAL_DUTIES}},"details":"HTS {{HTS_CODES}}, MFN {{MFN_RATE}}%, Sec301 {{SEC301_RATE}}%"}' >> ~/.pmlord/state/active-orders.jsonl
```
...
Route to `pmlord-escalate` with import compliance context.
...
Save as `PMLORD-ImportCompliance-{{YYYY-MM-DD}}-{{SLUG}}.docx` if report_format is docx.
...
```
pmlord-china-track (goods shipped from China)
  -> *** pmlord-import-compliance *** (HARD GATE — before customs entry)
    -> pmlord-logistics (customs clearance with classification data)
      -> pmlord-inspect (goods received at Rev A)
```
...
- **pmlord-profit:** Duty costs feed into actual vs estimated cost tracking
- **pmlord-logistics:** Classification data used for customs entry preparation
- **pmlord-pulse:** Alert on duty spikes, missing docs, AD/CVD flags
- **pmlord-crm-connector:** Duty cost data available for customer-facing reporting
- **pmlord-audit-trail:** All classification decisions logged
- **pmlord-change-order:** If specs change mid-stream, re-classify (HTS may change)
...
| Integration | API calls from PMLORD skill, results parsed and displayed |
...
- **Previous skill:** pmlord-china-track (goods shipped from partner)
- **Next skill:** pmlord-logistics (customs entry and clearance) → pmlord-inspect
- **On HOLD/ESCALATE:** pmlord-escalate with import compliance context
- **Data feed:** pmlord-profit (actual duty costs)
