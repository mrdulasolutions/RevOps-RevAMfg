...
version: 1.0.0
description: |
  Per-user voice and personality tuner for PMLORD. Captures each PM's
...
echo '{"skill":"pmlord-voice","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
Write two files to `~/.pmlord/users/{{PM_SLUG}}/`:
...
cat > ~/.pmlord/users/{{PM_SLUG}}/voice-profile.yaml << 'PROFILE'
[filled template content]
PROFILE

# Write preferences.yaml
cat > ~/.pmlord/users/{{PM_SLUG}}/preferences.yaml << 'PREFS'
[filled template content]
PREFS
```
...
```bash
touch ~/.pmlord/users/{{PM_SLUG}}/edit-history.jsonl
echo '{"event":"profile_created","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","version":"1.0.0"}' >> ~/.pmlord/users/{{PM_SLUG}}/edit-history.jsonl
```
...
Load the existing profile from `~/.pmlord/users/{{PM_SLUG}}/voice-profile.yaml`.
...
```bash
echo '{"event":"profile_edited","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","dimensions_changed":["{{DIMS}}"]}' >> ~/.pmlord/users/{{PM_SLUG}}/edit-history.jsonl
```
...
Read `~/.pmlord/users/{{PM_SLUG}}/voice-profile.yaml` and `preferences.yaml`.
...
Read `~/.pmlord/users/{{PM_SLUG}}/edit-history.jsonl`.
...
```bash
echo '{"event":"learn_update","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","pattern":"{{PATTERN}}","accepted":true}' >> ~/.pmlord/users/{{PM_SLUG}}/edit-history.jsonl
```
...
1. Read `~/.pmlord/config.yaml` to find the active PM slug
2. Check `~/.pmlord/users/{{PM_SLUG}}/voice-profile.yaml`
...
- `pmlord-customer-comms` — All customer emails
- `pmlord-rfq-quote` — Quote cover letters
- `pmlord-china-package` — Partner communications
- `pmlord-escalate` — Escalation notices
- `pmlord-report` — Report tone and detail level
- `pmlord-order-track` — Status update emails
- `pmlord-handoff` — Handoff communications
- `pmlord-ncr` — NCR notifications
...
4. Log to `~/.pmlord/users/{{PM_SLUG}}/edit-history.jsonl`:

```json
{
  "event": "content_edit",
  "ts": "2026-03-30T14:22:00Z",
  "skill": "pmlord-customer-comms",
...
- If `~/.pmlord/users/` directory does not exist, create it
