# pmlord-customer-gate

New customer onboarding gate skill for the PMLORD engine.
...
Typically triggered by `pmlord-rfq-qualify` when Gate 1 identifies a new customer. Can also be invoked directly.
...
- Customer onboarding record saved to `~/.pmlord/customers/{CUSTOMER_ID}/`
...
After onboarding completes, the skill routes back to `pmlord-rfq-quote` to continue the RFQ lifecycle.
