...
| **Entry criteria** | Manufacturing package sent to partner (via `/pmlord-china-package`). |
...
| **Key actions** | Run `/pmlord-inspect` for FAI. Document any deviations. Approve or request rework. |
...
| **Key actions** | Record tracking number. Calculate ETA at Rev A dock. Set up `/pmlord-inspect` for incoming inspection. Notify PM of expected arrival. |
