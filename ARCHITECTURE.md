...
PMLORD is a Claude Code skills engine that automates the Product Manager workflow at Rev A Manufacturing. It follows a modular, event-driven architecture with 27 skills organized into 9 groups.
...
│  pmlord-engine — intent routing, lifecycle      │
│  chaining, workflow state management            │
├─────────────────────────────────────────────────┤
│  Layer 3: Composition Skills                    │
│  pmlord-dashboard, pmlord-report,               │
│  pmlord-audit-trail — aggregate from atomics    │
├─────────────────────────────────────────────────┤
...
Email/Website/CRM
       │
       ▼
pmlord-rfq-intake ──▶ pmlord-rfq-qualify ──▶ pmlord-rfq-quote
       │                      │                      │
       ▼                      ▼                      ▼
  CRM record           Gate decision            Quote .docx
       ...
       │
       ▼
pmlord-china-package ──▶ pmlord-china-track ──▶ pmlord-inspect
       │                      │                      │
       ▼                      ▼                      ▼
  ...
  for China              (milestones)        (ACCEPT/REJECT)
       │
       ▼
pmlord-quality-gate ──▶ pmlord-repackage ──▶ pmlord-logistics
       │                      │                      │
       ▼                      ▼                      ▼
...
Events logged to `~/.pmlord/analytics/skill-usage.jsonl` in JSONL format.
Managed by `bin/pmlord-telemetry-log`.
...
4. Renames to `PMLORD-{Type}-{YYYY-MM-DD}-{ShortName}.docx`
5. Deletes temp `.md`
