# PMLORD Installation Guide

Four ways to install PMLORD into your Claude Code environment. Pick the one that fits your setup.
...
|-------------|-----|-------|
| **Claude Code** | PMLORD runs as a Claude Code skills engine | `claude --version` |
...
1. Creates `~/.pmlord/` runtime directory structure:
   ```
   ~/.pmlord/
     config.yaml
     sessions/
     analytics/
     state/
     reports/PMLORD-Reports/
     contributor-logs/
   ```

2. Symlinks the skill directory into Claude Code's skills path:
   ```
   ~/.claude/skills/pmlord -> /path/to/your/pmlord
   ```
...
5. Creates default configuration at `~/.pmlord/config.yaml`:
   ```yaml
   telemetry: off
   proactive: true
   report_format: docx
...
```bash
# Check the symlink
ls -la ~/.claude/skills/pmlord

# Check config
~/.claude/skills/pmlord/bin/pmlord-config list

# Launch PMLORD in Claude Code
claude
# Then type: /pmlord-engine
```
...
Let Claude Code install PMLORD for you. Open Claude Code and paste:

```
Clone the PMLORD skills engine from https://github.com/mrdulasolutions/RevOps-RevAMfg.git
and run the setup script. Install it as a Claude Code skills engine at ~/.claude/skills/pmlord.
```
...
4. You can immediately use `/pmlord-engine`
...
to install the PMLORD PM workflow engine.
```
...
3. Drag the `pmlord/` folder into the conversation
4. Ask Claude to run the setup script:
   ```
   Run the setup script in the pmlord directory I just uploaded.
   ```
...
cd /path/to/pmlord
zip -r pmlord-skills.zip . -x '*.git*' -x 'node_modules/*' -x '._*' -x '.DS_Store'
```
...
2. Upload `pmlord-skills.zip` or the extracted folder
3. Tell Claude:
   ```
   Extract pmlord-skills.zip to ~/.claude/skills/pmlord/ and run the setup script.
   ```
...
mkdir -p ~/.pmlord/{sessions,analytics,state,reports/PMLORD-Reports,contributor-logs}

# Create skills directory and link
mkdir -p ~/.claude/skills
ln -s /path/to/extracted/pmlord ~/.claude/skills/pmlord

# Make scripts executable
chmod +x ~/.claude/skills/pmlord/bin/*
chmod +x ~/.claude/skills/pmlord/setup

# Install docx dependencies (if package.json exists)
cd ~/.claude/skills/pmlord/pmlord-docx/scripts && npm install --silent 2>/dev/null || true

# Create default config
cat > ~/.pmlord/config.yaml << 'EOF'
telemetry: off
proactive: true
report_format: docx
crm_type: none
...
After installation, configure PMLORD for your environment:
...
# Set your CRM type
pmlord-config set crm_type dynamics    # or: powerapps, hubspot, none

# Enable telemetry (optional)
pmlord-config set telemetry community

# Set proactive mode
pmlord-config set proactive true       # auto-suggest skills based on context
```
...
3. Test with `/pmlord-crm-connector`
...
2. Test with `/pmlord-email-connector`
...
After installation, start PMLORD:
...
Your runtime data at `~/.pmlord/` (config, state, reports) is preserved across updates. Only the skill files are updated.
...
| Skills not found in Claude Code | Check symlink: `ls -la ~/.claude/skills/pmlord` |
| `.docx` reports not generating | Run `cd pmlord-docx/scripts && npm install` |
| Config not loading | Check `~/.pmlord/config.yaml` exists |
| Telemetry prompt keeps appearing | Run `touch ~/.pmlord/.telemetry-prompted` |
