...
Configure integrations with external systems — CRM, email, ERP, Slack, iMessage, and webhooks. Connector settings let PMLORD read from and push to your existing business tools.
...
- Dataverse table prefix: (default: `pmlord_`)
- Auth: "Set `PMLORD_DATAVERSE_CLIENT_ID` and `PMLORD_DATAVERSE_CLIENT_SECRET` in your shell profile"

**If B (Dynamics 365):**
- Organization URL
- Azure AD Tenant ID reference: `${PMLORD_AZURE_TENANT_ID}`
- Client ID reference: `${PMLORD_DYNAMICS_CLIENT_ID}`
- Client Secret reference: `${PMLORD_DYNAMICS_CLIENT_SECRET}`

**If C (HubSpot):**
- Portal ID
- API key reference: `${PMLORD_HUBSPOT_API_KEY}`
...
> Which email provider should PMLORD use for sending communications?
...
- Password reference: `${PMLORD_EMAIL_PASSWORD}`
...
**If C (Outlook):**
- Email address
- Tenant ID reference: `${PMLORD_MS_TENANT_ID}`
- Client ID reference: `${PMLORD_MS_CLIENT_ID}`
- Signature text
...
- Service layer URL
- Company DB name
- Auth: `${PMLORD_SAP_USER}` / `${PMLORD_SAP_PASSWORD}`
...
**If C (QuickBooks):**
- Company ID
- OAuth reference: `${PMLORD_QB_REFRESH_TOKEN}`
- Sync frequency
...
- Alert channel name (e.g., `#pmlord-alerts`)
- Orders channel name (e.g., `#orders`)
- Escalation channel name (e.g., `#escalation`)
- Bot token reference: `${PMLORD_SLACK_BOT_TOKEN}`
...
├──────────────────┴──────────────────────────────────┤
│  ENV VARS TO SET:                                   │
│  - PMLORD_HUBSPOT_API_KEY (if HubSpot)              │
│  - PMLORD_EMAIL_PASSWORD (if Hostinger)             │
│  - PMLORD_SLACK_BOT_TOKEN (if Slack)                │
...
Write to `~/.pmlord/config/connector-config.yaml` using `templates/connector-config.yaml.tmpl`.
