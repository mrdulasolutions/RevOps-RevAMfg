...
|------------|-------------|
| **ACCEPT** | "Parts accepted. Run `/reva-turbo-quality-gate` for G1 gate check? Or `/reva-turbo-repackage` to prepare for customer shipment?" |
| **REJECT** | "Parts rejected. Run `/reva-turbo-ncr` to create a Non-Conformance Report?" |
