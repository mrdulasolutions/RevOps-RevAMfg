# REVA-TURBO Configuration Schema Reference

Complete YAML schema documentation for all 6 configuration files written by `reva-turbo-setup`. All files are stored at `~/.reva-turbo/config/`.
...
| `footer_text` | string | yes | "Generated by REVA-TURBO \| {date}" | Report footer |
| `default_format` | string | yes | docx | docx/pdf/both |
| `templates_dir` | string | yes | ~/.reva-turbo/templates/ | Custom template directory |
| `output_dir` | string | yes | ~/Documents/REVA-TURBO-Reports/ | Local output directory |
...
| `backup.local_path` | string | no | ~/.reva-turbo/backups/ | Local backup directory |
| `backup.cloud_provider` | string | no | — | Cloud backup provider |
| `backup.cloud_path` | string | no | — | Cloud backup path |
| `naming_convention` | string | yes | "REVA-TURBO-{Type}-{YYYY-MM-DD}-{ShortName}" | File naming pattern |
