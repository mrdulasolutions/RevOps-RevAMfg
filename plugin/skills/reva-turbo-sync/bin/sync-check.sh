#!/usr/bin/env bash
# sync-check.sh — REVA-TURBO Sync Health Check
# Checks sync health: last successful sync per channel, pending items, failed items
# Usage: bash sync-check.sh

set -euo pipefail

SYNC_LOG="${HOME}/.reva-turbo/state/sync-log.jsonl"
SYNC_CONFIG="${HOME}/.reva-turbo/state/sync-config.yaml"
EMAIL_QUEUE="${HOME}/.reva-turbo/state/email-queue.jsonl"

echo "============================================"
echo "  REVA-TURBO Sync Health Check"
...
if [ ! -f "$SYNC_LOG" ]; then
    ...
    echo "  Sync has never been run. Run /reva-turbo-sync to configure."
    exit 0
fi
...
if [ ! -f "$SYNC_CONFIG" ]; then
    ...
    echo "  Sync is not configured. Run /reva-turbo-sync to set up."
    echo ""
fi
