...
echo '{"skill":"reva-turbo-reminder","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
When invoked (typically by reva-turbo-cron daily), scan all state files for reminders due today:

```bash
# Scan RFQ state for quote follow-ups
cat ~/.reva-turbo/state/workflow-state.jsonl 2>/dev/null | grep '"stage":"reva-turbo-rfq-quote"' | tail -50
...
cat ~/.reva-turbo/state/reminder-log.jsonl 2>/dev/null | tail -50
```
...
> **REVA-TURBO Reminders for Today ({{TODAY}}):**
>
...
2. Offer to invoke the relevant REVA-TURBO skill (e.g., reva-turbo-customer-comms for follow-up)
3. Log the reminder as processed

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","trigger":"{{TRIGGER_TYPE}}","ref":"{{REFERENCE_ID}}","customer":"{{CUSTOMER_COMPANY}}","action":"{{ACTION_TAKEN}}","pm":"{{PM_NAME}}","status":"{{STATUS}}"}' >> ~/.reva-turbo/state/reminder-log.jsonl
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","trigger":"{{TRIGGER_TYPE}}","ref":"{{REFERENCE_ID}}","action":"snoozed","snooze_until":"{{SNOOZE_DATE}}","pm":"{{PM_NAME}}"}' >> ~/.reva-turbo/state/reminder-log.jsonl
```

If dismissed:
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","trigger":"{{TRIGGER_TYPE}}","ref":"{{REFERENCE_ID}}","action":"dismissed","reason":"{{DISMISS_REASON}}","pm":"{{PM_NAME}}"}' >> ~/.reva-turbo/state/reminder-log.jsonl
```
