...
description: |
  ...
  routing, quality, workflow, and customer rules. The operating system layer of REVA-TURBO.
...
echo '{"skill":"reva-turbo-rules","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
Rules are stored in `~/.reva-turbo/rules.yaml` and evaluated by the rules engine (`bin/rules-engine.sh`).

## Flow

### Step 0: Mode Selection

> **REVA-TURBO Rules Engine**
>
...
> 3. **Alert rule** -- condition-based notifications via reva-turbo-pulse
...
>    (Examples: reva-turbo-rfq-quote, reva-turbo-order-track, all)
...
```bash
mkdir -p ~/.reva-turbo
if [ ! -f ~/.reva-turbo/rules.yaml ]; then
  echo "rules: []" > ~/.reva-turbo/rules.yaml
fi
```
...
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"rule_created","rule_id":"{{RULE_ID}}","rule_name":"{{RULE_NAME}}","rule_type":"{{RULE_TYPE}}","created_by":"{{PM_NAME}}","active":{{ACTIVE}}}' >> ~/.reva-turbo/state/rules-log.jsonl 2>/dev/null || true
```
...
```bash
cat ~/.reva-turbo/rules.yaml 2>/dev/null || echo "No rules file found. Create rules first."
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"rule_edited","rule_id":"{{RULE_ID}}","edited_by":"{{PM_NAME}}","fields_changed":"{{CHANGED_FIELDS}}"}' >> ~/.reva-turbo/state/rules-log.jsonl 2>/dev/null || true
```
...
```bash
cat ~/.reva-turbo/rules.yaml 2>/dev/null || echo "No rules file found."
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"rule_toggled","rule_id":"{{RULE_ID}}","new_status":"{{NEW_STATUS}}","toggled_by":"{{PM_NAME}}"}' >> ~/.reva-turbo/state/rules-log.jsonl 2>/dev/null || true
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"rule_deleted","rule_id":"{{RULE_ID}}","deleted_by":"{{PM_NAME}}","rule_name":"{{RULE_NAME}}"}' >> ~/.reva-turbo/state/rules-log.jsonl 2>/dev/null || true
```
...
```bash
cat ~/.reva-turbo/rules.yaml 2>/dev/null || echo "No rules defined yet."
```
...
```bash
grep '"status":"complete"' ~/.reva-turbo/state/workflow-state.jsonl 2>/dev/null | tail -20 || echo "No historical orders found"
```
...
```bash
echo '{{TEST_CONTEXT_JSON}}' | bash ~/.reva-turbo/bin/rules-engine.sh --test --rule-id "{{RULE_ID}}"
```
...
```bash
tail -50 ~/.reva-turbo/state/rules-eval-log.jsonl 2>/dev/null || echo "No rule evaluations logged yet."
```
...
When any REVA-TURBO skill runs, it should check for applicable rules by calling the rules engine:

```bash
echo '{{CURRENT_CONTEXT_JSON}}' | bash ~/.reva-turbo/bin/rules-engine.sh --scope "{{SKILL_NAME}}"
```
...
4. Send alerts via reva-turbo-pulse
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"rule_evaluation","rule_id":"{{RULE_ID}}","rule_name":"{{RULE_NAME}}","triggered":{{TRIGGERED}},"context_skill":"{{SKILL_NAME}}","context_order":"{{ORDER_ID}}","context_customer":"{{CUSTOMER}}","action_taken":"{{ACTION_TAKEN}}"}' >> ~/.reva-turbo/state/rules-eval-log.jsonl 2>/dev/null || true
```

## Rule Storage

Rules are stored in `~/.reva-turbo/rules.yaml`. See `references/rule-syntax.md` for the complete schema.
