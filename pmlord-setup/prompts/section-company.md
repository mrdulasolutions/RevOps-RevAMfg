...
Collect the company's identity, contact information, team roster, and escalation matrix. This information drives report headers, email signatures, CRM records, and escalation routing across all PMLORD skills.
...
The PMLORD escalation matrix has 4 levels. Each level needs a name and email. The person can be from the team roster or someone not yet listed.
...
Write to `~/.pmlord/config/company-profile.yaml` using `templates/company-profile.yaml.tmpl`.
