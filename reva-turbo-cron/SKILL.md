...
version: 1.0.0
description: |
  Scheduled task management for the REVA-TURBO engine. Defines and manages
...
Manage scheduled and recurring tasks for the REVA-TURBO engine. Define what needs to run, when it runs, and track execution history. This skill acts as the scheduler that triggers other REVA-TURBO skills on a defined cadence.
...
cat ~/.reva-turbo/state/cron-log.jsonl 2>/dev/null | tail -20
```
...
3. Invoke the appropriate REVA-TURBO skill
4. Log the execution
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","task":"{{TASK_ID}}","skill":"{{TRIGGERED_SKILL}}","status":"{{STATUS}}","pm":"{{PM_NAME}}"}' >> ~/.reva-turbo/state/cron-log.jsonl
```
...
cat ~/.reva-turbo/state/cron-log.jsonl 2>/dev/null | grep "$(date -u +%Y-%m)" | tail -50
```
...
|------|---------|-----|----------------|
| Daily Order Status Check | Daily | Every day | reva-turbo-order-track |
| Weekly Partner Follow-Up | Weekly | Monday | reva-turbo-partner-scorecard |
| Weekly PM Summary | Weekly | Friday | reva-turbo-report |
| Monthly Partner Scorecards | Monthly | 1st business day | reva-turbo-partner-scorecard |
| Monthly Performance Report | Monthly | 1st business day | reva-turbo-report |
| Quarterly Business Review | Quarterly | 1st week of Q | reva-turbo-report |
