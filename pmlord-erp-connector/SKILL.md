---
name: pmlord-erp-connector
preamble-tier: 2
version: 1.0.0
description: |
  Connect PMLORD to ERP and inventory systems for capacity data, material
...
```bash
cat ~/.pmlord/config/erp-config.json 2>/dev/null || echo '{"erp_type":"none"}'
```
...
ls ~/.pmlord/imports/*.csv 2>/dev/null
```

Parse CSV files into PMLORD data structures.
...
Process retrieved data and make it available to other PMLORD skills:

```bash
mkdir -p ~/.pmlord/erp-data
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"{{DATA_TYPE}}","source":"{{ERP_SOURCE}}","records":{{RECORD_COUNT}}}' >> ~/.pmlord/erp-data/erp-sync-log.jsonl
```
...
```bash
# Capacity data
echo '{{CAPACITY_JSON}}' > ~/.pmlord/erp-data/capacity.json

# Cost data
echo '{{COST_JSON}}' > ~/.pmlord/erp-data/costs.json

# Inventory data
echo '{{INVENTORY_JSON}}' > ~/.pmlord/erp-data/inventory.json
```
...
2. **No direct ERP writes.** PMLORD reads from ERP. Writes go through CRM or are manual.
