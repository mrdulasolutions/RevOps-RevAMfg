...
Audit trail management skill for the REVA-TURBO engine.

## What It Does

Maintains a complete, immutable record of every significant PM action and decision:

1. Logs audit entries from all REVA-TURBO skills (automatic) and manual PM entries
...
Or ask REVA-TURBO to log a decision, query the audit trail, or generate an audit report.
...
Audit entries are stored at `~/.reva-turbo/state/audit-trail.jsonl` as newline-delimited JSON.
