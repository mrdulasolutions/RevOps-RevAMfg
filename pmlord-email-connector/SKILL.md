...
version: 1.2.0
description: |
  Universal email connector for PMLORD. Auto-detects available email providers
  ...
  PMLORD skill. Supports Hostinger, Gmail, Microsoft 365/Outlook, and any
  MCP-connected inbox.
...
```bash
mkdir -p ~/.pmlord/analytics ~/.pmlord/state
echo '{"skill":"pmlord-email-connector","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
_EMAIL_PROVIDER=$(~/.claude/skills/pmlord/bin/pmlord-config get email_provider 2>/dev/null || echo "auto")
echo "EMAIL_PROVIDER: $_EMAIL_PROVIDER"
```

## Purpose

Universal email connector for Rev A Manufacturing PMs. Connects to ANY email provider available in the Claude instance — Hostinger (revamfg.com), Gmail, Microsoft 365/Outlook, or any MCP-connected inbox. Reads, sends, replies, searches, classifies, and routes emails to the appropriate PMLORD skill.
...
**How PMLORD uses it:** When this connector is enabled, simply ask Claude to search or read Outlook emails directly. No MCP tool calls needed — Claude's native M365 integration handles retrieval. PMLORD classifies and routes the results.
...
Since the native M365 connector is read-only, PMLORD uses these providers for sending, replying, and advanced email management:
...
|----------|------------------|----------|
| **RFQ / Quote Request** | "quote", "RFQ", "pricing", "request for quote", "bid" | pmlord-rfq-intake |
| **Order Update** | "PO", "order status", references existing PO number | pmlord-order-track |
| **Partner Update** | From known partner domain, production/QC status | pmlord-china-track |
| **Quality Issue** | "defect", "NCR", "quality", "complaint", "damaged" | pmlord-ncr / pmlord-escalate |
| **Shipping Update** | "tracking", "AWB", "B/L", "customs", carrier domains | pmlord-logistics |
| **Customer Inquiry** | General question, follow-up, relationship | pmlord-customer-comms |
| **Payment / Invoice** | "invoice", "payment", "overdue", "remittance" | pmlord-order-track |
| **Compliance** | "ITAR", "EAR", "export license", "HTS", "tariff" | pmlord-export-compliance / pmlord-import-compliance |
...
For each confirmed email, invoke the target PMLORD skill with the extracted data.

Log the routing:
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","provider":"{{PROVIDER}}","from":"{{SENDER}}","subject":"{{SUBJECT}}","category":"{{CATEGORY}}","routed_to":"{{TARGET_SKILL}}","pm":"{{PM_NAME}}","message_id":"{{MESSAGE_ID}}"}' >> ~/.pmlord/state/email-routing-log.jsonl
```
...
### mode: send
Send an email on behalf of the PM. Used by pmlord-customer-comms and other skills.
...
```bash
~/.claude/skills/pmlord/bin/pmlord-config set email_provider "{{PROVIDER}}"
```
...
- **Move to folder:** `mcp__hostinger-email__move_email` (e.g., move processed emails to "PMLORD-Processed")
...
1. **Mark as processed** — Mark read or move to PMLORD-Processed folder (Hostinger) or apply label (Gmail)
2. **Log routing** — Append to `~/.pmlord/state/email-routing-log.jsonl`
3. **Dedup check** — Before processing, check log for message_id to avoid duplicate processing
4. **Pulse alert** — If any email is CRITICAL urgency, trigger `pmlord-pulse` alert
