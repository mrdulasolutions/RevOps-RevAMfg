...
Full order lifecycle tracking skill for the PMLORD engine.
...
Or ask PMLORD to check on an order by PO number or customer name.
...
- Order state log at `~/.pmlord/orders/{PO_NUMBER}/order-state.jsonl`
...
When delays exceed 2 weeks, the skill recommends running `pmlord-escalate`. When inspection is needed, it connects with `pmlord-inspect`. For repackaging, it triggers `pmlord-repackage`.
