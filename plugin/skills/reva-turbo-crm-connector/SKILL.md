---
name: reva-turbo-crm-connector
preamble-tier: 2
version: 1.0.0
description: |
  Sync REVA-TURBO data with CRM systems. Supports Microsoft Power Apps / Dynamics 365
  and HubSpot. Read and write customer, order, and RFQ data. Maps REVA-TURBO fields
...
echo '{"skill":"reva-turbo-crm-connector","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```

## Purpose

Provide bidirectional synchronization between REVA-TURBO and CRM systems. Read customer data, order history, and contact information from CRM. Write back order status updates, RFQ outcomes, and interaction logs. Supports Microsoft Dynamics 365 / Power Apps and HubSpot.
...
```bash
cat ~/.reva-turbo/config/crm-config.json 2>/dev/null || echo '{"crm_type":"none"}'
```
...
> B) HubSpot
> C) None — use local REVA-TURBO state files only
...
> E) Sync REVA-TURBO data to CRM
> F) Pull CRM data into REVA-TURBO
> G) Check sync status
...
When syncing REVA-TURBO data to CRM:

1. Read current REVA-TURBO state for the entity
2. Map REVA-TURBO fields to CRM fields
3. Check for conflicts (CRM data newer than REVA-TURBO data)
...
> **CRM Sync Plan:**
>
> Direction: {{SYNC_DIRECTION}} (REVA-TURBO -> CRM / CRM -> REVA-TURBO)
> Entity: {{ENTITY_TYPE}} ({{ENTITY_NAME}})
> Fields to update: {{FIELD_COUNT}}
>
> | Field | REVA-TURBO Value | CRM Value | Action |
> |-------|-------------|-----------|--------|
> | {{FIELD_ROW_1}} |
> | {{FIELD_ROW_2}} |
>
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","action":"crm_sync","direction":"{{DIRECTION}}","entity":"{{ENTITY_TYPE}}","name":"{{ENTITY_NAME}}","fields_updated":{{FIELD_COUNT}},"crm_type":"{{CRM_TYPE}}","pm":"{{PM_NAME}}"}' >> ~/.reva-turbo/state/crm-sync-log.jsonl
```

## Rules

1. **Never overwrite CRM data without PM confirmation.** Always show what will change.
2. **Conflict resolution is manual.** If REVA-TURBO and CRM have different values, present both and let the PM decide.
...
5. **Graceful degradation.** If CRM is unreachable, fall back to local REVA-TURBO state files.
