---
name: reva-turbo-trust
preamble-tier: 2
version: 1.0.0
description: |
  Progressive autonomy system for REVA-TURBO. Three trust levels control
...
echo '{"skill":"reva-turbo-trust","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```

## Purpose

Trust is the behavioral backbone of the REVA-TURBO engine. It does not gate access or lock features -- it modifies HOW every skill communicates, confirms, and executes. A new PM gets a patient teaching assistant. An experienced PM gets a silent operator that only speaks up when something goes wrong.
...
```bash
cat ~/.reva-turbo/state/trust-level.json 2>/dev/null || echo '{"level":1,"name":"learn","set_by":"default","set_at":"never"}'
```
...
```bash
echo '{"level":{{LEVEL}},"name":"{{NAME}}","set_by":"{{PM_NAME}}","set_at":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","previous_level":{{PREV_LEVEL}},"reason":"{{REASON}}"}' > ~/.reva-turbo/state/trust-level.json
```

4. Log the change to history:

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","action":"set","level":{{LEVEL}},"name":"{{NAME}}","set_by":"{{PM_NAME}}","scope":"system","reason":"{{REASON}}"}' >> ~/.reva-turbo/state/trust-history.jsonl
```
...
```bash
cat ~/.reva-turbo/state/trust-level.json 2>/dev/null || echo "none"
```
...
**Question 4:**
> Do you want to learn how the REVA-TURBO engine works as you use it?
>
...
```bash
cat ~/.reva-turbo/state/trust-history.jsonl 2>/dev/null || echo "No trust history found."
```
...
```bash
cat ~/.reva-turbo/state/trust-overrides.jsonl 2>/dev/null || echo "No overrides set."
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"{{skill|entity}}","skill":"{{SKILL_NAME}}","entity_type":"{{ENTITY_TYPE}}","entity_id":"{{ENTITY_ID}}","level":{{LEVEL}},"reason":"{{REASON}}","expires":"{{EXPIRY_DATE|null}}","set_by":"{{PM_NAME}}"}' >> ~/.reva-turbo/state/trust-overrides.jsonl
```

7. Log to history:

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","action":"override","level":{{LEVEL}},"name":"{{NAME}}","scope":"{{SCOPE}}","reason":"{{REASON}}"}' >> ~/.reva-turbo/state/trust-history.jsonl
```
...
All trust state lives under `~/.reva-turbo/state/`:
...
Per-user trust levels are stored at `~/.reva-turbo/users/<pm-slug>/trust-level.json` using the same schema as the system-wide file.

## Integration with Other Skills

Every REVA-TURBO skill should call `bin/trust-check.sh` at the start to determine the effective trust level, then apply the behavioral overlay from the appropriate prompt file:
...
If no trust level is set (`~/.reva-turbo/state/trust-level.json` does not exist), the engine defaults to **Level 1: LEARN** and offers to run the trust assessment:

> Welcome to REVA-TURBO. Since this is your first time, I am starting in **LEARN mode** -- I will explain each step as we go.
>
