...
The design philosophy behind the PMLORD engine. These principles shaped every skill, every template, every decision gate, and every line of this system. They are non-negotiable.
...
The output of PMLORD is documents, not chat messages. Quotes are `.docx` files. Reports are `.docx` files. Inspection checklists are `.docx` files. NCRs are `.docx` files.
...
The `{{PLACEHOLDER}}` template system ensures consistency. The DOCX converter ensures professionalism. The naming convention (`PMLORD-{Type}-{YYYY-MM-DD}-{ShortName}.docx`) ensures findability.

---

## 5. The PM's Time is Sacred

Every feature in PMLORD exists to save PM time or prevent PM error. If a feature does neither, it doesn't belong.

- **Quick** exists because typing `/pmlord-rfq-intake` and filling 15 fields takes 10 minutes. Typing "quote Acme 500 aluminum brackets" takes 5 seconds.
...
- **Sync** exists because entering the same data in PMLORD and CRM and email is triple-entry. The system should do that.
...
PMLORD uses structured prompts (`AskUserQuestion` with lettered options) at every decision point. This isn't limiting — it's liberating. PMs don't have to remember what information to collect. The skill tells them.

---

## 7. Fail Loudly, Never Silently

If something goes wrong, PMLORD tells you. Immediately.
...
Silent failures are how manufacturing companies lose money, customers, and reputation. PMLORD does not fail silently.
...
This means:
- A PM can use just `/pmlord-rfq-quote` without the rest of the engine
- A PM can use just `/pmlord-inspect` for receiving
- A PM can run the full lifecycle through `/pmlord-engine`
- New skills can be added without modifying existing skills
- Skills can be updated independently

The `conductor.json` file defines the wiring between skills. The `pmlord-engine` orchestrator reads it. But each skill's `SKILL.md` contains everything needed to run that skill in isolation.

---

## 9. Connectors, Not Lock-In

PMLORD connects to external systems (CRM, email, ERP) through adapter patterns that support multiple backends.
...
PMLORD is not a generic project management tool. It is built specifically for contract manufacturing with China-based supply chains.
...
Generic tools don't know these things. PMLORD does.

---

## 11. The Engine Learns

Every order that flows through PMLORD makes the system smarter:
...
The engine doesn't just process — it accumulates institutional intelligence. The longer Rev A uses PMLORD, the more accurate its predictions, the tighter its estimates, the faster its decisions.
...
The best PM with PMLORD should be 3x the PM they were without it — not because the AI is smarter, but because the AI handles the 80% that doesn't require human judgment, freeing the PM to excel at the 20% that does.

---

**These 12 principles are the foundation of PMLORD. Every skill is measured against them. Every feature must serve at least one. If it doesn't, it doesn't ship.**
