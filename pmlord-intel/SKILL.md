...
description: |
  ...
  Transforms PMLORD from a logger into an advisor — predicting delays,
...
echo '{"skill":"pmlord-intel","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```

## Purpose

Intel transforms PMLORD's historical data into forward-looking intelligence. Every JSONL log, every scorecard entry, every audit trail record is a data point. Intel reads the patterns, calculates the probabilities, and gives the PM advice they can act on — not just data they have to interpret.
...
> **PMLORD Intel — What do you want to know?**
>
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","pm":"{{PM_NAME}}","analysis_type":"{{ANALYSIS_TYPE}}","scope":"{{SCOPE}}","time_period":"{{TIME_PERIOD}}","entity":"{{ENTITY_FILTER}}","comparison":"{{COMPARISON_TYPE}}"}' >> ~/.pmlord/analytics/intel-requests.jsonl 2>/dev/null || true
```
...
|-------------|-----------|-------------------|
| Skill usage | `~/.pmlord/analytics/skill-usage.jsonl` | PM activity patterns, skill frequency |
| Workflow state | `~/.pmlord/state/workflow-state.jsonl` | Stage durations, transition patterns, bottlenecks |
| Active orders | `~/.pmlord/state/active-orders.jsonl` | Current pipeline, stage distribution |
| Active RFQs | `~/.pmlord/state/active-rfqs.jsonl` | Quote pipeline, conversion tracking |
| NCR log | `~/.pmlord/state/ncr-log.jsonl` | Defect types, partner quality, cost of poor quality |
| Partner scores | `~/.pmlord/state/partner-scores.jsonl` | Partner trends, comparative performance |
| Inspection records | `~/.pmlord/state/inspection-log.jsonl` | First pass yield, defect Pareto |
| Invoice tracker | `~/.pmlord/state/invoices.jsonl` | Revenue, payment patterns, DSO |
| Escalation log | `~/.pmlord/state/escalations.jsonl` | Issue frequency, resolution patterns |
| Customer profiles | `~/.pmlord/state/customer-profiles.jsonl` | CLV, order history, churn signals |
| Pulse alerts | `~/.pmlord/state/pulse-alerts.jsonl` | Alert frequency, resolution times |
| Autopilot log | `~/.pmlord/state/autopilot-config.jsonl` | Automation adoption, time savings |
| Carryover log | `~/.pmlord/state/carryover-log.jsonl` | Workflow velocity |
| Audit trail | `~/.pmlord/state/audit-trail.jsonl` | Decision history, compliance |
...
Run all five modes and compile into a comprehensive report using `templates/Intel Report.md`. Generate as markdown, and offer to convert to DOCX via pmlord-docx.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","pm":"{{PM_NAME}}","analysis_type":"{{ANALYSIS_TYPE}}","scope":"{{SCOPE}}","data_points":{{DATA_POINTS}},"confidence":"{{CONFIDENCE}}","key_finding":"{{KEY_FINDING_SUMMARY}}"}' >> ~/.pmlord/analytics/intel-outputs.jsonl 2>/dev/null || true
```
...
```
PMLORD-Intel-{YYYY-MM-DD}-{AnalysisType}-{Scope}.md
```
