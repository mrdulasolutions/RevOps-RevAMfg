...
Configure the REVA-TURBO lifecycle pipeline — which stages are active, custom display names, quality gate thresholds, auto-advance rules, priority definitions, and SLA targets. This drives the entire order lifecycle engine.
...
```
REVA-TURBO Lifecycle Pipeline — 15 Stages
...
> REVA-TURBO uses 4 priority levels. Confirm or customize response time expectations:
>
...
Write to `~/.reva-turbo/config/workflow-config.yaml` using `templates/workflow-config.yaml.tmpl`.
