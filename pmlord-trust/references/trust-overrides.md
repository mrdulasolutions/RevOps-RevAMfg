...
**Location:** `~/.pmlord/state/trust-level.json`
...
**Location:** `~/.pmlord/users/<pm-slug>/trust-level.json`
...
**Location:** `~/.pmlord/state/trust-overrides.jsonl` (append-only)

```json
{
  "ts": "2026-03-15T14:35:00Z",
  "type": "skill",
  "skill": "pmlord-export-compliance",
  "entity_type": null,
  "entity_id": null,
  "level": 1,
...
|---|---|---|
| `pmlord-export-compliance` | Level 1 | Safety-critical, want to understand every decision |
| `pmlord-import-compliance` | Level 1 | Complex regulations, want full context |
| `pmlord-rfq-intake` | Level 3 | High volume, routine process |
| `pmlord-customer-comms` | Level 2 | Want to review communications but not draft from scratch |
| `pmlord-china-track` | Level 3 | Routine tracking, auto-update customers |
| `pmlord-report` | Level 3 | Auto-generate standard reports |
...
Skill override: `pmlord-export-compliance` -> Level 1 LEARN
...
grep '"active":true' ~/.pmlord/state/trust-overrides.jsonl 2>/dev/null | while read line; do
...
```bash
cat ~/.pmlord/state/trust-overrides.jsonl | jq -s 'sort_by(.ts)'
```
...
|---|---|---|
| `pmlord-export-compliance` | Level 2 ASSIST | Export compliance decisions have legal consequences. AI classification and license determination must always be reviewed by a human. |
| `pmlord-import-compliance` | Level 2 ASSIST | Import classification and formal customs entries require human review for legal compliance. |
| `pmlord-quality-gate` (final inspection) | Level 2 ASSIST | Final inspection sign-off determines whether product ships to customer. Must have human review. |
| `pmlord-inspect` (lot acceptance) | Level 2 ASSIST | Lot acceptance/rejection decisions affect customer satisfaction and have financial impact. |
