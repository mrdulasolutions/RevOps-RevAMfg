...
| **Entry criteria** | Manufacturing package sent to partner (via `/reva-turbo:reva-turbo-china-package`). |
...
| **Key actions** | Run `/reva-turbo:reva-turbo-inspect` for FAI. Document any deviations. Approve or request rework. |
...
| **Key actions** | Record tracking number. Calculate ETA at Rev A dock. Set up `/reva-turbo:reva-turbo-inspect` for incoming inspection. Notify PM of expected arrival. |
