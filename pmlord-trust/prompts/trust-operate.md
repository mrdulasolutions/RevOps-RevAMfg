...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"{{SKILL}}","action":"{{ACTION}}","entity":"{{ENTITY_ID}}","result":"{{RESULT}}","trust_level":3,"autonomous":true}' >> ~/.pmlord/analytics/trust-audit.jsonl
```
...
These thresholds are stored in `~/.pmlord/state/trust-thresholds.json` and can be adjusted:
