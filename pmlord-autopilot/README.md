...
Invisible workflow transitions for the PMLORD engine. The skill that makes PMLORD feel alive.
...
- Autopilot configuration saved to `~/.pmlord/state/autopilot-config.jsonl`
- Auto-transition log entries in `~/.pmlord/state/workflow-state.jsonl`
- Data carryover log in `~/.pmlord/state/carryover-log.jsonl`
- Override log in `~/.pmlord/state/autopilot-overrides.jsonl`
