#!/usr/bin/env bash
# pulse-check.sh — Scan PMLORD state files for alert-triggering conditions
# Called by pmlord-cron on a schedule or manually via /pmlord-pulse
#
...
# The calling skill (pmlord-pulse) handles delivery and filtering.
#
# Exit codes:
    ...
    while IFS= read -r line; do
        ...
        if [ -n "$ncr_ts" ] && [ "$ncr_status" = "open" ]; then
            ...
            if [ "$days_open" -le 1 ]; then
                alert_id=$(next_alert_id)
                echo '{"ts":"'"$NOW"'","alert_id":"'"$alert_id"'","alert_type":"NCR_ISSUED","alert_code":"ALT-002","priority":"CRITICAL","entity_id":"'"$entity_id"'","entity_type":"order","customer":"'"$customer"'","message":"New NCR '"$ncr_id"' issued for '"$entity_id"'","pm":"'"$PM_NAME"'","status":"pending","action_required":"Review NCR and determine disposition","relevant_skill":"pmlord-ncr"}'
            fi
        fi
    done < "$NCR_FILE"
fi
...
if [ -f "$ORDERS_FILE" ]; then
    ...
    if [ "$active_count" -gt 15 ]; then
        alert_id=$(next_alert_id)
        echo '{"ts":"'"$NOW"'","alert_id":"'"$alert_id"'","alert_type":"CAPACITY_WARNING","alert_code":"ALT-010","priority":"WARNING","entity_id":"'"$PM_NAME"'","entity_type":"pm","message":"PM '"$PM_NAME"' has '"$active_count"' active orders (>15 threshold)","pm":"'"$PM_NAME"'","status":"pending","action_required":"Consider redistributing orders or requesting support","relevant_skill":"pmlord-dashboard"}'
    fi
fi
