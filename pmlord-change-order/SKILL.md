...
echo '{"skill":"pmlord-change-order","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```

## Purpose

Manage mid-stream changes to in-progress orders. Calculate full impact across cost, lead time, tooling, quality, partner operations, and logistics. Get PM approval with complete visibility. Cascade approved changes through all downstream PMLORD skills. Maintain full audit trail with revision control.
...
```bash
cat ~/.pmlord/state/orders/{{ORDER_ID}}.yaml 2>/dev/null || echo "Order not found."
```
...
Route through `/pmlord-customer-comms` for sending.
...
Route through `/pmlord-customer-comms` (partner mode).

### Step 6 — Cascade Updates

Automatically update all affected PMLORD records:

| Record | Update Action | Skill |
|--------|-------------|-------|
| Quote | Revise with new pricing, mark as "Revised" | `/pmlord-rfq-quote` |
| Order | Update specs, timeline, revision level | `/pmlord-order-track` |
| China-track | Adjust milestones per new timeline | `/pmlord-china-track` |
| Inspection | Update checklist with new criteria | `/pmlord-inspect` |
| Logistics | Update shipping plan (weight, dims, dates) | `/pmlord-logistics` |
| Customer profile | Log change order in activity history | `/pmlord-customer-profile` |
| Partner scorecard | Log change event (partner-initiated only) | `/pmlord-partner-scorecard` |
| Audit trail | Full change order record | `/pmlord-audit-trail` |

Each cascade update is logged:

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","co_number":"{{CO_NUMBER}}","cascade":"{{TARGET_SKILL}}","entity":"{{ENTITY_ID}}","action":"{{UPDATE_ACTION}}","status":"success"}' >> ~/.pmlord/state/change-order-log.jsonl
```
...
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","co_number":"{{CO_NUMBER}}","order_id":"{{ORDER_ID}}","customer":"{{CUSTOMER}}","change_type":"{{CHANGE_TYPE}}","original_value":"{{ORIGINAL_VALUE}}","new_value":"{{NEW_VALUE}}","reason":"{{REASON}}","requested_by":"{{REQUESTED_BY}}","cost_delta":"{{DELTA_TOTAL}}","time_delta":"{{DELTA_DELIVERY}}","tooling_impact":"{{TOOLING_SUMMARY}}","decision":"{{DECISION}}","decided_by":"{{PM_NAME}}","revision":"{{REVISION}}"}' >> ~/.pmlord/state/change-order-log.jsonl
```

Also log in the main audit trail:

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"change_order","ref":"{{ORDER_ID}}","customer":"{{CUSTOMER}}","action":"Change order {{CO_NUMBER}}: {{CHANGE_TYPE}} — {{DECISION}}","justification":"{{REASON}}","pm":"{{PM_NAME}}","skill":"pmlord-change-order"}' >> ~/.pmlord/state/audit-trail.jsonl
```
...
```bash
LAST_SEQ=$(grep '"order_id":"{{ORDER_ID}}"' ~/.pmlord/state/change-order-log.jsonl 2>/dev/null | wc -l | tr -d ' ')
...
> B) Initiate full re-quote via `/pmlord-rfq-quote`
...
4. Name the file: `PMLORD-CO-{{CO_NUMBER}}.md`

## Report Naming

```
PMLORD-CO-{CO-NUMBER}.md
```

Example: `PMLORD-CO-CO-ORD-2026-0142-01.md`

## State Tracking

Change order log: `~/.pmlord/state/change-order-log.jsonl`
Audit trail: `~/.pmlord/state/audit-trail.jsonl`
