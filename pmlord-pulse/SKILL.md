...
echo '{"skill":"pmlord-pulse","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```

## Purpose

Pulse is the PM's nervous system. It continuously monitors PMLORD's state files for exception conditions and delivers targeted alerts through the PM's preferred channels. Unlike a dashboard that requires the PM to look, Pulse pushes information to the PM when they need to act.
...
> E) In-system only (alerts stored in PMLORD, visible on next interaction)
>
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","pm":"{{PM_NAME}}","alert_config":"{{ALERT_CONFIG_JSON}}"}' >> ~/.pmlord/state/pulse-alert-config.jsonl 2>/dev/null || true
```

### Step 3 — Alert Detection Scan

Scan PMLORD state files for alert-triggering conditions:

**Data sources to scan:**

| Source | File | What to look for |
|--------|------|-----------------|
| Workflow state | `~/.pmlord/state/workflow-state.jsonl` | Stale stages (no progress >3 days) |
| Active orders | `~/.pmlord/state/active-orders.jsonl` | Overdue milestones, approaching deadlines |
| Active RFQs | `~/.pmlord/state/active-rfqs.jsonl` | Expiring quotes, no-response RFQs |
| NCR log | `~/.pmlord/state/ncr-log.jsonl` | New NCRs, unresolved NCRs |
| Partner scorecards | `~/.pmlord/state/partner-scores.jsonl` | Score drops, low scores |
| Inspection schedule | `~/.pmlord/state/inspection-schedule.jsonl` | Upcoming inspections |
| Invoice tracker | `~/.pmlord/state/invoices.jsonl` | Overdue payments |
| Escalation log | `~/.pmlord/state/escalations.jsonl` | Active escalations |
| Skill usage | `~/.pmlord/analytics/skill-usage.jsonl` | PM workload (count active entities) |
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","alert_id":"PULSE-{{ALERT_SEQ}}","alert_type":"{{ALERT_TYPE}}","priority":"{{PRIORITY}}","entity_id":"{{ENTITY_ID}}","entity_type":"{{ENTITY_TYPE}}","message":"{{ALERT_MESSAGE}}","pm":"{{PM_NAME}}","status":"pending","channels_targeted":["{{CHANNELS}}"],"action_required":"{{ACTION_DESCRIPTION}}"}' >> ~/.pmlord/state/pulse-alerts.jsonl 2>/dev/null || true
```
...
2. If PM interacts with PMLORD before check-in: present warnings at start of session
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","alert_id":"{{ALERT_ID}}","action":"{{PM_ACTION}}","pm":"{{PM_NAME}}","snooze_until":"{{SNOOZE_UNTIL}}"}' >> ~/.pmlord/state/pulse-actions.jsonl 2>/dev/null || true
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","digest_type":"daily","pm":"{{PM_NAME}}","alerts_included":{{ALERT_COUNT}},"pending_actions":{{PENDING_COUNT}},"deadlines_today":{{DEADLINE_COUNT}}}' >> ~/.pmlord/state/pulse-digests.jsonl 2>/dev/null || true
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","digest_type":"weekly","pm":"{{PM_NAME}}","week_start":"{{WEEK_START_DATE}}","total_alerts":{{TOTAL_ALERTS}},"critical":{{CRITICAL_COUNT}},"warning":{{WARNING_COUNT}},"info":{{INFO_COUNT}},"avg_resolution_minutes":{{AVG_RESOLUTION}}}' >> ~/.pmlord/state/pulse-digests.jsonl 2>/dev/null || true
```
