...
echo '{"skill":"reva-turbo-rfq-qualify","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
This skill expects an intake record from `reva-turbo-rfq-intake`. If no intake record exists, prompt the PM to run intake first or provide the RFQ details manually.

## Flow

### Step 1: Load Intake Record

Read the intake record from `~/.reva-turbo/rfqs/{{RFQ_ID}}/intake-record.md` or accept the data passed from the intake skill. Confirm the following minimum fields are populated:
...
**Returning customer:**
- Check for existing customer profile in `~/.reva-turbo/customers/`
...
**New customer:**
- Flag for onboarding via `reva-turbo-customer-gate`
...
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","rfq_id":"{{RFQ_ID}}","customer":"{{CUSTOMER_COMPANY}}","stage":"reva-turbo-rfq-qualify","status":"{{DECISION}}","pm":"{{ASSIGNED_PM}}","next_suggested":"NEXT_SKILL"}' >> ~/.reva-turbo/state/workflow-state.jsonl 2>/dev/null || true
```

**Routing based on decision:**

- **PROCEED** — Route to `reva-turbo-rfq-quote`
- **CONDITIONAL (new customer)** — Route to `reva-turbo-customer-gate` first, then `reva-turbo-rfq-quote`
- **CONDITIONAL (other)** — Route to `reva-turbo-rfq-quote` with conditions noted
- **DECLINE** — Route to `reva-turbo-customer-comms` to draft a professional decline communication
...
```
REVA-TURBO-RFQQualify-{YYYY-MM-DD}-{CustomerShortName}.md
```
