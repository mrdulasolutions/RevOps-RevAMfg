...
Real-time alert feed and PM nervous system for the REVA-TURBO engine.

## What It Does

Continuously monitors REVA-TURBO state files for exception conditions and pushes targeted alerts to the PM through their preferred channels. Pulse is not noisy — it only speaks up when something matters.
...
Or Pulse runs automatically in the background via reva-turbo-cron.
...
- Alert records in `~/.reva-turbo/state/pulse-alerts.jsonl`
- Alert actions in `~/.reva-turbo/state/pulse-actions.jsonl`
- Digest records in `~/.reva-turbo/state/pulse-digests.jsonl`
