...
description: |
  ...
  capacity utilization. Aggregates data from all REVA-TURBO state files.
...
echo '{"skill":"reva-turbo-dashboard","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```

## Purpose

Provide PMs (Ray Yeh, Harley Scott) and leadership (Donovan Weber) with a real-time workload dashboard. Aggregates data from all REVA-TURBO state files to show a comprehensive view of open RFQs, active orders, quality issues, upcoming deliveries, partner performance, and capacity.
...
Gather data from REVA-TURBO state files:

```bash
# Count active orders by stage
cat ~/.reva-turbo/state/workflow-state.jsonl 2>/dev/null | grep '"status":"active"' | tail -100

# Count open RFQs
cat ~/.reva-turbo/state/workflow-state.jsonl 2>/dev/null | grep '"stage":"reva-turbo-rfq' | grep -v '"status":"closed"' | tail -50

# Get quality issues
cat ~/.reva-turbo/state/ncr-log.jsonl 2>/dev/null | grep '"status":"open"' | tail -20

# Get upcoming deliveries
cat ~/.reva-turbo/shipments/shipment-log.jsonl 2>/dev/null | tail -30

# Get partner scores
cat ~/.reva-turbo/state/partner-scores.jsonl 2>/dev/null | tail -20

# Get escalations
cat ~/.reva-turbo/state/escalation-log.jsonl 2>/dev/null | grep '"status":"open"' | tail -10
```
...
```bash
mkdir -p ~/.reva-turbo/dashboards
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"pm-dashboard","pm":"{{ASSIGNED_PM}}","active_orders":{{ACTIVE_ORDERS}},"open_rfqs":{{OPEN_RFQS}},"open_ncrs":{{OPEN_NCRS}},"open_escalations":{{OPEN_ESCALATIONS}}}' >> ~/.reva-turbo/dashboards/dashboard-log.jsonl
```
...
```
REVA-TURBO-Dashboard-{YYYY-MM-DD}-{PM_INITIALS}.md
```
