...
**Data Source:** `~/.reva-turbo/orders/*/order-state.jsonl` — compare `customer_delivery_date` (promised) vs actual delivery timestamp at Stage 11.
...
**Data Source:** `~/.reva-turbo/state/ncr-log.jsonl` cross-referenced with completed orders.
...
**Data Source:** `~/.reva-turbo/state/workflow-state.jsonl` — timestamp delta between `reva-turbo-rfq-intake` complete and `reva-turbo-rfq-quote` complete.
...
**Data Source:** `~/.reva-turbo/state/workflow-state.jsonl` — count of RFQs reaching "won" status vs total reaching "quoted" status.
...
**Data Source:** `~/.reva-turbo/orders/*/order-state.jsonl` — timestamp delta between Stage 01 and Stage 11.
...
**Data Source:** `~/.reva-turbo/state/partner-scores.jsonl`
...
**Data Source:** `~/.reva-turbo/state/escalation-log.jsonl`
...
**Data Source:** `~/.reva-turbo/state/workflow-state.jsonl` filtered by `pm` field and `status:"active"`.
...
**Data Source:** `~/.reva-turbo/rfqs/*/intake-record.md` — extract estimated value field.
