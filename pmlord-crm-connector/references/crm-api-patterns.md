...
PMLORD uses HubSpot MCP tools for direct integration:
...
| PMLORD Field | HubSpot Property | Object Type |
|-------------|-----------------|-------------|
...
Create these custom properties in HubSpot for PMLORD integration:

| Property | Internal Name | Object | Type | Description |
|----------|-------------|--------|------|-------------|
| PO Number | `pmlord_po_number` | Deal | Single-line text | Rev A PO number |
| PMLORD Stage | `pmlord_stage` | Deal | Single-line text | Order stage (01-12) |
| Routing Type | `pmlord_routing` | Deal | Dropdown | direct-to-customer / inspect-and-forward |
| Mfg Partner | `pmlord_mfg_partner` | Deal | Single-line text | China partner name |
| Delivery Date | `pmlord_delivery_date` | Deal | Date | Promised delivery date |
| Partner Score | `pmlord_partner_score` | Company | Single-line text | Partner grade (vendor companies) |
| Last Sync | `pmlord_last_sync` | All | Date/time | Last PMLORD sync timestamp |

### HubSpot Pipeline Stages

Map PMLORD order stages to HubSpot deal pipeline stages:

| PMLORD Stage | HubSpot Stage | Stage ID |
|-------------|--------------|----------|
...
For Dynamics 365, PMLORD uses the Dataverse Web API. See `dynamics-field-mapping.md` for field details.
...
2. Compare with PMLORD state files
...
### Event-Driven Sync
1. Triggered when a PMLORD skill completes an action (e.g., stage transition)
