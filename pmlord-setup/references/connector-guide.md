...
3. Copy the key and set it as an environment variable:
   ```bash
   export PMLORD_HUBSPOT_API_KEY="pat-na1-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
   ```
...
### Field Mapping
PMLORD maps to HubSpot deal properties:
| PMLORD Field | HubSpot Property | Notes |
|---|---|---|
...
| Priority | Custom property `pmlord_priority` | Create this property |
...
6. Set environment variables:
   ```bash
   export PMLORD_AZURE_TENANT_ID="your-tenant-id"
   export PMLORD_DYNAMICS_CLIENT_ID="your-client-id"
   export PMLORD_DYNAMICS_CLIENT_SECRET="your-client-secret"
   ```
...
3. Set environment variables:
   ```bash
   export PMLORD_DATAVERSE_CLIENT_ID="your-client-id"
   export PMLORD_DATAVERSE_CLIENT_SECRET="your-client-secret"
   ```

### Table Prefix
PMLORD creates Dataverse tables with a configurable prefix (default: `pmlord_`):
- `pmlord_rfq` — RFQ records
- `pmlord_order` — Order tracking
- `pmlord_partner` — Partner scorecard data
...
### Configuration in PMLORD
- Set the sending address (Gmail address)
- Configure signature text
- PMLORD uses the Gmail MCP tool for send/read operations
...
```bash
export PMLORD_EMAIL_PASSWORD="your-email-password"
```
...
3. Set environment variables:
   ```bash
   export PMLORD_MS_TENANT_ID="your-tenant-id"
   export PMLORD_MS_CLIENT_ID="your-client-id"
   ```
...
8. Set environment variable:
   ```bash
   export PMLORD_SLACK_BOT_TOKEN="xoxb-xxxxxxxxxxxx-xxxxxxxxxxxx-xxxxxxxxxxxxxxxxxxxxxxxx"
   ```

### Recommended Channels
Create these channels in your Slack workspace:
- `#pmlord-alerts` — Automated alerts (SLA warnings, escalations)
- `#pmlord-orders` — Order status updates
- `#pmlord-escalation` — Escalation notifications (restricted to managers)
...
### Connection
```bash
export PMLORD_SAP_USER="your-sap-username"
export PMLORD_SAP_PASSWORD="your-sap-password"
```
...
4. Set environment variables:
   ```bash
   export PMLORD_NETSUITE_ACCOUNT_ID="your-account-id"
   export PMLORD_NETSUITE_TOKEN_ID="your-token-id"
   export PMLORD_NETSUITE_TOKEN_SECRET="your-token-secret"
   ```
...
5. Set environment variables:
   ```bash
   export PMLORD_QB_CLIENT_ID="your-client-id"
   export PMLORD_QB_CLIENT_SECRET="your-client-secret"
   export PMLORD_QB_REFRESH_TOKEN="your-refresh-token"
   export PMLORD_QB_COMPANY_ID="your-company-id"
   ```
