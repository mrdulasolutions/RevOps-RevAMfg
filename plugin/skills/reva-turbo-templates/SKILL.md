---
name: reva-turbo-templates
preamble-tier: 2
version: 1.0.0
description: |
  Central template management for the REVA-TURBO engine. Maintains an inventory
...
echo '{"skill":"reva-turbo-templates","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```

## Purpose

Serve as the central registry for all REVA-TURBO templates. Every template used across every skill is cataloged here with its location, version, purpose, and key variables. This skill helps PMs find the right template and ensures consistency across the engine.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","template":"{{TEMPLATE_NAME}}","skill":"{{SOURCE_SKILL}}","pm":"{{PM_NAME}}"}' >> ~/.reva-turbo/state/template-usage.jsonl 2>/dev/null || true
```
...
5. **Report naming convention.** All generated reports follow: `REVA-TURBO-{Type}-{YYYY-MM-DD}-{ShortName}.md` or `.docx`
