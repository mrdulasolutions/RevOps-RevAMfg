...
echo '{"skill":"pmlord-escalate","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"escalation","ref":"{{REFERENCE_ID}}","customer":"{{CUSTOMER_COMPANY}}","trigger":"{{TRIGGER_TYPE}}","level":"{{ESCALATION_LEVEL}}","escalated_to":"{{ESCALATED_TO}}","pm":"{{PM_NAME}}","status":"open","severity":"{{SEVERITY}}"}' >> ~/.pmlord/state/escalation-log.jsonl
```

Also log in the audit trail:

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"escalation","ref":"{{REFERENCE_ID}}","customer":"{{CUSTOMER_COMPANY}}","action":"Escalated to {{ESCALATED_TO}}: {{TRIGGER_TYPE}}","justification":"","pm":"{{PM_NAME}}","skill":"pmlord-escalate"}' >> ~/.pmlord/state/audit-trail.jsonl
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"escalation_resolved","ref":"{{REFERENCE_ID}}","resolution":"{{RESOLUTION}}","resolved_by":"{{RESOLVED_BY}}","days_to_resolve":{{DAYS_TO_RESOLVE}}}' >> ~/.pmlord/state/escalation-log.jsonl
```
...
```
PMLORD-Escalation-{YYYY-MM-DD}-{Reference}.md
```
