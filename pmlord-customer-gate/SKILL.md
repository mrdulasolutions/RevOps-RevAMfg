...
echo '{"skill":"pmlord-customer-gate","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
This skill is triggered when `pmlord-rfq-qualify` identifies a new customer (Gate 1 result = NEW). It can also be invoked directly when a PM needs to onboard a customer outside the RFQ flow.
...
echo '{"action":"crm_setup","customer":"{{CUSTOMER_COMPANY}}","customer_id":"{{CUSTOMER_ID}}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/state/crm-push-log.jsonl 2>/dev/null || true
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","customer":"{{CUSTOMER_COMPANY}}","customer_id":"{{CUSTOMER_ID}}","stage":"pmlord-customer-gate","status":"complete","pm":"{{ASSIGNED_PM}}","next_suggested":"pmlord-rfq-quote"}' >> ~/.pmlord/state/workflow-state.jsonl 2>/dev/null || true
```
...
>
> Next step: Return to **quote generation** (pmlord-rfq-quote) for RFQ {{RFQ_ID}}. Want me to run it?

## Report Naming

```
PMLORD-CustGate-{YYYY-MM-DD}-{CustomerShortName}.md
```
