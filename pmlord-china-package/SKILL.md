...
echo '{"skill":"pmlord-china-package","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
3. Name the file: `PMLORD-MFG-PKG-{{DATE}}-{{PART_NAME}}.md`
...
> "Manufacturing package is ready. Next step: `/pmlord-china-track` to set up milestone tracking for this order. Want me to run it?"

## Report Naming

`PMLORD-MFG-PKG-{YYYY-MM-DD}-{PartName}.docx`
