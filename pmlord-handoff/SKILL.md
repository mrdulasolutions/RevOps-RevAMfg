...
echo '{"skill":"pmlord-handoff","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
grep '"pm":"{{DEPARTING_PM}}"' ~/.pmlord/state/workflow-state.jsonl 2>/dev/null | grep -v '"status":"complete"' | grep -v '"status":"cancelled"' || echo "No active items found in state file"
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"vacation_handoff","from":"{{DEPARTING_PM}}","to":"{{BACKUP_PM}}","start":"{{VACATION_START}}","end":"{{VACATION_END}}","items_transferred":{{ITEM_COUNT}},"auto_revert":"{{VACATION_END}}"}' >> ~/.pmlord/state/handoff-log.jsonl 2>/dev/null || true
```

Create a revert trigger:

```bash
echo '{"revert_date":"{{VACATION_END}}","from":"{{BACKUP_PM}}","to":"{{DEPARTING_PM}}","handoff_id":"{{HANDOFF_ID}}","status":"pending"}' >> ~/.pmlord/state/handoff-reverts.jsonl 2>/dev/null || true
```

Update pulse alert routing:

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"alert_redirect","original_pm":"{{DEPARTING_PM}}","redirect_to":"{{BACKUP_PM}}","until":"{{VACATION_END}}"}' >> ~/.pmlord/state/alert-routing.jsonl 2>/dev/null || true
```
...
> - Handoff briefing saved to `~/.pmlord/handoffs/{{HANDOFF_ID}}/`
>
> {{BACKUP_PM}}: your handoff briefing is ready. Run `/pmlord-handoff` and select "review incoming handoff" to see it.

#### Step A6: Auto-Revert (on return date)

When the return date arrives (checked by `pmlord-cron` or `pmlord-pulse`):
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"permanent_transfer","from":"{{FROM_PM}}","to":"{{TO_PM}}","items":{{TRANSFER_ITEMS_JSON}},"reason":"{{TRANSFER_REASON}}"}' >> ~/.pmlord/state/handoff-log.jsonl 2>/dev/null || true
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"shared_ownership","item_id":"{{ITEM_ID}}","primary_pm":"{{PRIMARY_PM}}","secondary_pm":"{{SECONDARY_PM}}","secondary_role":"{{SECONDARY_ROLE}}","duration":"{{DURATION}}"}' >> ~/.pmlord/state/handoff-log.jsonl 2>/dev/null || true
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","type":"approval","item_id":"{{ITEM_ID}}","action":"{{APPROVAL_TYPE}}","value":"{{APPROVAL_VALUE}}","requested_by":"{{REQUESTING_PM}}","approver":"{{APPROVER}}","decision":"{{DECISION}}","conditions":"{{CONDITIONS}}","reason":"{{REASON}}"}' >> ~/.pmlord/state/approval-log.jsonl 2>/dev/null || true
```
...
```bash
grep -c '"pm":"{{PM_NAME}}"' ~/.pmlord/state/workflow-state.jsonl 2>/dev/null || echo "0"
```
...
All handoff logs stored at `~/.pmlord/state/handoff-log.jsonl`.
All approval logs stored at `~/.pmlord/state/approval-log.jsonl`.
