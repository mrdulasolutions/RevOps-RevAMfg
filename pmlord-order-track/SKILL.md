...
echo '{"skill":"pmlord-order-track","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
cat ~/.pmlord/orders/*/order-state.jsonl 2>/dev/null | grep -i "{{SEARCH_TERM}}" | tail -20
```
...
```bash
mkdir -p ~/.pmlord/orders/{{PO_NUMBER}}
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","po":"{{PO_NUMBER}}","customer":"{{CUSTOMER_COMPANY}}","stage":"01-po-received","status":"active","pm":"{{ASSIGNED_PM}}","routing":"{{ROUTING_TYPE}}"}' >> ~/.pmlord/orders/{{PO_NUMBER}}/order-state.jsonl
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","po":"{{PO_NUMBER}}","from_stage":"{{PREVIOUS_STAGE}}","to_stage":"{{NEW_STAGE}}","triggered_by":"{{TRIGGER}}","next_expected":"{{NEXT_EXPECTED_DATE}}","pm":"{{ASSIGNED_PM}}"}' >> ~/.pmlord/orders/{{PO_NUMBER}}/order-state.jsonl
```
...
3. Determine if escalation is needed (delay > 2 weeks triggers pmlord-escalate)
4. Draft customer communication if delivery date is affected

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","po":"{{PO_NUMBER}}","event":"delay","stage":"{{CURRENT_STAGE}}","reason":"{{DELAY_REASON}}","original_date":"{{ORIGINAL_DATE}}","revised_date":"{{REVISED_DATE}}","days_delayed":{{DAYS_DELAYED}},"escalation_required":{{ESCALATION_BOOL}}}' >> ~/.pmlord/orders/{{PO_NUMBER}}/order-state.jsonl
```

If escalation is required:

> **Escalation Required:** This delay of {{DAYS_DELAYED}} days exceeds the 2-week threshold. Recommend running pmlord-escalate. Proceed?
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"pmlord-order-track","po":"{{PO_NUMBER}}","action":"{{ACTION}}","pm":"{{ASSIGNED_PM}}","detail":"{{DETAIL}}"}' >> ~/.pmlord/state/audit-trail.jsonl 2>/dev/null || true
```
...
```
PMLORD-OrderStatus-{YYYY-MM-DD}-{PO_NUMBER}.md
```
