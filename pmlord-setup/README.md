...
Interactive onboarding wizard for the PMLORD engine. Walks the PM through 7 configuration sections and generates 6 YAML config files that drive the entire Rev A Manufacturing workflow.

## When to Use

- First-time PMLORD setup on a new machine or for a new company
- Reconfiguring a specific section (e.g., adding a new manufacturing partner)
- After upgrading PMLORD to pick up new config options

## Invocation

```
/pmlord-setup              # Start full wizard (or resume if partial)
/pmlord-setup section:3    # Jump to a specific section
/setup                     # Alias (routed by pmlord-engine)
```
...
All config files are written to `~/.pmlord/config/`:
...
- Skipped sections can be configured later via `/pmlord-setup section:N`.
