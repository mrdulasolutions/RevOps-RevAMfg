...
Build the manufacturing partner roster — the factories, suppliers, and subcontractors that PMLORD will use for quoting, ordering, and quality tracking. Each partner gets a detailed profile with capabilities, lead times, quality ratings, and contact information.

This data drives:
- `pmlord-china-package` — knows which partners can handle which part types
- `pmlord-rfq-quote` — pulls lead times and quality ratings for quoting
- `pmlord-partner-scorecard` — tracks partner performance over time
- `pmlord-pulse` — sends alerts through preferred communication channels
...
Write to `~/.pmlord/config/partners.yaml` using `templates/partners.yaml.tmpl`.
