...
4. If status is **Critical**, suggest escalation: "This delay may impact the customer delivery date. Escalate to Senior PM? (`/reva-turbo-escalate`)"
...
|------------------|-----------------|
| M3 (First article) | "First article is ready. Run `/reva-turbo-inspect` to set up the FAI checklist?" |
| M8 (QC passed) | "QC passed. Run `/reva-turbo-quality-gate` for G1 incoming gate?" |
| M10 (Shipped) | "Order shipped. Run `/reva-turbo-logistics` to track shipment?" |
| Any delay | "Delay detected. Run `/reva-turbo-escalate` to notify stakeholders?" |
