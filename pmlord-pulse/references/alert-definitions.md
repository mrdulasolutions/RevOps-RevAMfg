...
| ALT-004 | ESCALATION_TRIGGERED | Any escalation event logged by pmlord-escalate or auto-detected | CRITICAL | Slack + iMessage | Immediate PM attention. Review escalation details and determine response within 1 hour. |
...
| ALT-013 | NEW_RFQ_RECEIVED | A new RFQ has been detected in email or entered via CRM | INFO | Slack | Process through pmlord-rfq-intake when ready. No urgency unless customer flagged it as rush. |
...
Each alert record in `~/.pmlord/state/pulse-alerts.jsonl` follows this schema:
...
```json
{
  ...
  "relevant_skill": "pmlord-china-track",
  "repeat_count": 1,
...
Custom rules are stored in `~/.pmlord/state/pulse-custom-rules.jsonl`.
