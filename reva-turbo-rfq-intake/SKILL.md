...
echo '{"skill":"reva-turbo-rfq-intake","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
echo '{"action":"crm_push","skill":"reva-turbo-rfq-intake","rfq_id":"{{RFQ_ID}}","customer":"{{CUSTOMER_COMPANY}}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.reva-turbo/state/crm-push-log.jsonl 2>/dev/null || true
```

Reference `references/rfq-field-mapping.md` for CRM field mapping. If CRM is not configured, save the intake record to:

```
~/.reva-turbo/rfqs/{{RFQ_ID}}/intake-record.md
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","rfq_id":"{{RFQ_ID}}","customer":"{{CUSTOMER_COMPANY}}","stage":"reva-turbo-rfq-intake","status":"complete","pm":"{{ASSIGNED_PM}}","next_suggested":"reva-turbo-rfq-qualify"}' >> ~/.reva-turbo/state/workflow-state.jsonl 2>/dev/null || true
```

Then suggest the next step:

> RFQ intake complete for {{CUSTOMER_COMPANY}}. Next step: **Qualification gate check** (reva-turbo-rfq-qualify). Want me to run it?

## Report Naming

Save the intake record as:
```
REVA-TURBO-RFQIntake-{YYYY-MM-DD}-{CustomerShortName}.md
```
