...
`~/.reva-turbo/preferences.yaml`

## Full Schema

```yaml
# ~/.reva-turbo/preferences.yaml
# PM-specific preferences for REVA-TURBO quick commands
...
- Via file: Edit `~/.reva-turbo/preferences.yaml` directly
...
Control how and when REVA-TURBO notifies the PM:
...
If `~/.reva-turbo/preferences.yaml` doesn't exist, create it with sensible defaults on first use:

```bash
mkdir -p ~/.reva-turbo
if [ ! -f ~/.reva-turbo/preferences.yaml ]; then
  cat > ~/.reva-turbo/preferences.yaml << 'PREFS'
pm_name: ""
pm_email: ""
default_margin: 25
