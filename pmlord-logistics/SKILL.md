...
echo '{"skill":"pmlord-logistics","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
mkdir -p ~/.pmlord/shipments
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","po":"{{PO_NUMBER}}","type":"{{SHIP_TYPE}}","mode":"{{SHIPPING_MODE}}","carrier":"{{CARRIER}}","tracking":"{{TRACKING_NUMBER}}","origin":"{{ORIGIN}}","destination":"{{DESTINATION}}","etd":"{{ETD}}","eta":"{{ETA}}","routing":"{{ROUTING_TYPE}}"}' >> ~/.pmlord/shipments/shipment-log.jsonl
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","po":"{{PO_NUMBER}}","event":"shipping_exception","type":"{{EXCEPTION_TYPE}}","detail":"{{EXCEPTION_DETAIL}}","impact_days":{{IMPACT_DAYS}}}' >> ~/.pmlord/shipments/shipment-log.jsonl
```
...
```
PMLORD-Shipping-{YYYY-MM-DD}-{PO_NUMBER}.md
```
