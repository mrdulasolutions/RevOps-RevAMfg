...
Track progression metrics in `~/.reva-turbo/analytics/trust-progression.jsonl`:
...
Each PM has their own trust level stored at `~/.reva-turbo/users/<pm-slug>/trust-level.json`. A senior PM might be at Level 3 while a new PM is at Level 1. The engine adjusts behavior based on who is using it.
