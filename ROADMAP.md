...
| Component | Status |
|-----------|--------|
| Orchestrator (pmlord-engine) | Shipped |
| Config system (pmlord-config) | Shipped |
...
- API routes reading from PMLORD data layer (`~/.pmlord/state/`)
...
**PMLORD integration:**
- Portal writes new RFQs → `pmlord-rfq-intake` picks them up
- Portal reads order status from `~/.pmlord/state/active-orders.jsonl`
- Portal serves documents from `~/.pmlord/reports/PMLORD-Reports/`
- Portal triggers `pmlord-pulse` alerts on customer actions (RFQ submitted, quote accepted)
- Portal reads customer profile from `pmlord-customer-profile` data
- PM uses `pmlord-customer-comms` → message appears in portal
...
#### Option B: PMLORD Skill + Generated Portal

A PMLORD skill (`pmlord-customer-portal`) that generates per-customer status pages. The PM runs `/pmlord-customer-portal` and gets a shareable link or HTML file for that customer.
...
**Technical approach:**
- Skill generates static HTML from PMLORD state data
...
**Phase 1 (this session):** Build `pmlord-customer-portal` as a PMLORD skill (Option B). This gives Rev A an immediate capability — PMs can generate customer-facing status pages today.
...
| Multi-tenant support | Architecture | Run PMLORD for multiple clients from one engine |
| AI quote estimation | Intelligence | Train on historical quotes to auto-estimate new ones |
| Voice interface | Platform | "Hey PMLORD, what's the status of Acme's order?" |
