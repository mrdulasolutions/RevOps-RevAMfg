...
if [ -z "$PM_SLUG" ] && [ -x "$PMLORD_CONFIG" ]; then
  DEFAULT_PM=$("$PMLORD_CONFIG" get default_pm 2>/dev/null || echo "")
...
VOICE_FILE="${HOME}/.pmlord/users/${PM_SLUG}/voice-profile.yaml"
PREFS_FILE="${HOME}/.pmlord/users/${PM_SLUG}/preferences.yaml"
