...
echo '{"skill":"pmlord-rfq-quote","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.pmlord/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
This skill expects a qualified RFQ from `pmlord-rfq-qualify`. The qualification decision should be PROCEED or CONDITIONAL (with conditions addressed). If no qualification record exists, warn the PM:
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","rfq_id":"{{RFQ_ID}}","customer":"{{CUSTOMER_COMPANY}}","stage":"pmlord-rfq-quote","status":"complete","quote_number":"{{QUOTE_NUMBER}}","total":"{{TOTAL_QUOTE}}","pm":"{{ASSIGNED_PM}}","next_suggested":"pmlord-customer-comms"}' >> ~/.pmlord/state/workflow-state.jsonl 2>/dev/null || true
```

Suggest the next step:

> Quote {{QUOTE_NUMBER}} generated for {{CUSTOMER_COMPANY}} — total ${{TOTAL_QUOTE}}.
> Next step: **Send quote to customer** (pmlord-customer-comms). Want me to draft the quote submission email?

## Report Naming

```
PMLORD-Quote-{YYYY-MM-DD}-{CustomerShortName}.md
```
