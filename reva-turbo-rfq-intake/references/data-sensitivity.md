...
Rules for handling customer data during RFQ intake and throughout the REVA-TURBO workflow.
...
**Handling:** Store in REVA-TURBO state directory (`~/.reva-turbo/`). Do not write to shared/public directories. Do not include in logs that may be shared.
...
REVA-TURBO analytics and telemetry logs must never contain:
- Customer names
- Part descriptions
- Drawing content
- Pricing data
...
- Physical files remain in the PM's email/file system — REVA-TURBO tracks references only
...
2. Note the incident in `~/.reva-turbo/state/incident-log.jsonl`
