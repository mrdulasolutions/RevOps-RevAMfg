...
|---|-------|------|------|-----|-------|
| 1 | Incoming inspection report completed (`/reva-turbo-inspect`) | | | | |
...
| **G3 PASS** | "G3 passed. Run G4 (pre-ship gate) when ready to ship? Or `/reva-turbo-repackage` first?" |
| **G4 PASS** | "G4 passed. Parts cleared for shipment. Run `/reva-turbo-logistics` to arrange shipping?" |
| **CONDITIONAL PASS** | "Conditional pass. Set reminder for condition deadline via `/reva-turbo-reminder`?" |
| **FAIL** | "Gate failed. Run `/reva-turbo-ncr` for non-conformance report? Or `/reva-turbo-escalate` if urgent?" |
